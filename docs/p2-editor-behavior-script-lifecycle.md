...
- 默认产品使用两个 Script v2 dependency、Scene/KSCN v9 和 Host Interface v4；Scene v9 在保留全部 Behavior/Prototype/Tilemap 语义的同时增加静态 Camera2D。`player_controller.luau` 除既有 Player fixed movement、decoration frame update 与 contact/custom event 外，还通过 `runtime-orb` Prototype 证明 transient spawn、动态 `on_start`、后续 callback 与 destroy/stale 生命周期。
