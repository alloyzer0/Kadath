...
public_branch=$(kcov_metric public-c percent_covered_branches) || block 'public C branch denominator missing'
zig_line=$(kcov_metric zig-adapter percent_covered) || block 'Zig Adapter line denominator missing'
zig_branch=$(kcov_metric zig-adapter percent_covered_branches) || block 'Zig Adapter branch denominator missing'
...
meets "$public_branch" 85 || block 'public C branch coverage below 85%'
meets "$zig_line" 90 || block 'Zig Adapter line coverage below 90%'
meets "$zig_branch" 85 || block 'Zig Adapter branch coverage below 85%'
...
printf 'timer_priority\tRust unit coverage binary\tCOVERED\n' >>"$decision_manifest"
...
printf 'stale_contact\tRust unit coverage binary\tCOVERED\n' >>"$decision_manifest"
...
printf 'paired_candidate\truntime-core-contract\tCOVERED\n' >>"$decision_manifest"
...
printf 'restart_high_water\truntime-core-contract\tCOVERED\n' >>"$decision_manifest"
...
printf 'phase_capacity\truntime-core-contract\tCOVERED\n' >>"$decision_manifest"
decision_total=$(awk 'NR > 1 { total++ } END { print total+0 }' "$decision_manifest")
decision_covered=$(awk 'NR > 1 && $3 == "COVERED" { covered++ } END { print covered+0 }' "$decision_manifest")
...
GAMEPLAY_COVERAGE_PUBLIC_C_BRANCH_PERCENT=$public_branch
GAMEPLAY_COVERAGE_ZIG_ADAPTER_LINE_PERCENT=$zig_line
GAMEPLAY_COVERAGE_ZIG_ADAPTER_BRANCH_PERCENT=$zig_branch
