...
$texturePath = Join-Path $package 'bin\assets\renderer2d\test.texture'
...
try {
    ...
    if ($artifact.Length -lt 24 -or [Text.Encoding]::ASCII.GetString($artifact, 0, 4) -cne 'KDAT') { throw 'Runtime texture artifact has an invalid KDAT header' }
    ...
    if ($version -ne 2 -or $mipLevelCount -ne 2 -or $pixelBytes -ne 20 -or $artifact.Length -ne 44) { throw 'Runtime texture artifact is not the expected KDAT v2 mip chain' }
    ...
    if (-not ($runtimeLog.Contains('Renderer2D texture upload complete: mip_levels=2, sampler=smooth_mipmap_anisotropic'))) { throw 'Renderer2D anisotropic sampler request evidence is missing' }
    if (-not ($runtimeLog.Contains('sampler_requested=smooth_mipmap_anisotropic'))) { throw 'RHI anisotropic sampler request evidence is missing' }
