...
function Get-ArtifactInfo([string]$Path, [ValidateSet('Scene', 'Script')][string]$Kind) {
    ...
    if ($Kind -eq 'Scene') {
        if ($bytes.Length -ne 128 -or [Text.Encoding]::ASCII.GetString($bytes, 0, 4) -cne 'KSCN') {
            throw 'Scene artifact layout mismatch'
        }
        if ([BitConverter]::ToUInt32($bytes, 4) -ne 1 -or [BitConverter]::ToUInt32($bytes, 8) -ne 1 -or [BitConverter]::ToUInt32($bytes, 12) -ne 112) {
