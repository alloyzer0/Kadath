...
function Assert-BuildContract([string]$ContractDirectory, [string]$ExpectedProfile, [switch]$IncludeCompatibilityPpm) {
    ...
    if ([string]$contract.ProfileSemantics.debug -notlike '*KDAT Texture Artifact v1 base-only*' -or [string]$contract.ProfileSemantics.release -notlike '*KDAT Texture Artifact v2 mipmap chain*' -or [string]$contract.ProfileSemantics.debug -notlike '*KSCN Scene Artifact v3*' -or [string]$contract.ProfileSemantics.release -notlike '*KSCN Scene Artifact v3*' -or [string]$contract.ProfileSemantics.debug -notlike '*KSCP Script Artifact v1*' -or [string]$contract.ProfileSemantics.release -notlike '*KSCP Script Artifact v1*') { throw 'Build Contract profile semantics do not describe texture/scene/script transforms' }
    ...
    $expectedContracts = @{
        ...
        'bin/assets/scenes/preview.scene.json' = @{ OutputPath = 'bin/assets/scenes/preview.scene'; InputArtifactType = 'RuntimeSceneDocumentV3'; ArtifactType = 'RuntimeSceneArtifactV3'; Transform = 'scene-json-to-kscn-v3'; ImporterStatus = 'implemented-v3'; BakerStatus = 'implemented-v3' }
...
try {
    ...
    if ([string]$plan.ProfileSemantics.debug -notlike '*KDAT Texture Artifact v1 base-only*' -or [string]$plan.ProfileSemantics.release -notlike '*KDAT Texture Artifact v2 mipmap chain*' -or [string]$plan.ProfileSemantics.debug -notlike '*KSCN Scene Artifact v3*' -or [string]$plan.ProfileSemantics.release -notlike '*KSCN Scene Artifact v3*' -or [string]$plan.ProfileSemantics.debug -notlike '*KSCP Script Artifact v1*' -or [string]$plan.ProfileSemantics.release -notlike '*KSCP Script Artifact v1*') { throw 'Build Contract dry-run profile semantics are invalid' }
