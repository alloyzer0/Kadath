...
fn main() {
    let header = PathBuf::from("../../abi/kadath_world.h");
    println!("cargo:rerun-if-changed={}", header.display());
    println!("cargo:rerun-if-changed=../../abi/kadath_errors.h");

    // 本机 Windows 开发环境未把 LLVM 加入 PATH，给 bindgen 一个可移植前的保守回退。
    ...
    let bindings = bindgen::Builder::default()
        ...
        .clang_arg("-I../../abi")
        .allowlist_type("kadath_.*")
        .allowlist_var("KADATH_.*")
        .allowlist_function("kadath_world_.*")
        .derive_default(true)
        .generate()
        .expect("failed to generate kadath_world C ABI bindings");

    let out_path = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR must be set"));
    bindings
        .write_to_file(out_path.join("kadath_world_bindings.rs"))
        .expect("failed to write kadath_world bindings");
}
